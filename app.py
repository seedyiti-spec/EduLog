...
KEY_NAMES = ["GEMINI_API_KEY", "NOTION_TOKEN", "NOTION_DATABASE_ID"]
...
def load_keys_from_file() -> dict:
    """Read keys from .streamlit/secrets.toml directly (bypasses Streamlit cache)."""
...
def save_keys_to_file(keys: dict):
    """Write all keys to .streamlit/secrets.toml."""
...
def init_session_keys():
    """Load persisted keys into session state exactly once per session."""
    if st.session_state.get("_keys_initialized"):
        return
    saved = load_keys_from_file()
    for key in KEY_NAMES:
        # Priority: secrets.toml file → env var (don't overwrite existing session state)
        val = saved.get(key, "") or os.environ.get(key, "")
        st.session_state[f"_input_{key}"] = val
    st.session_state["_last_saved"] = {k: st.session_state[f"_input_{k}"] for k in KEY_NAMES}
    st.session_state["_keys_initialized"] = True


def check_and_persist_keys() -> bool:
    """Compare current inputs to last saved state; write file if changed. Returns True if just saved."""
    current = {k: st.session_state.get(f"_input_{k}", "") for k in KEY_NAMES}
    last = st.session_state.get("_last_saved", {})
    if current != last:
        save_keys_to_file(current)
        st.session_state["_last_saved"] = current.copy()
        return True
    return False


def get_keys() -> dict:
    return {k: st.session_state.get(f"_input_{k}", "") for k in KEY_NAMES}
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
def render_sidebar():
    # Detect any key change and persist before rendering status
    just_saved = check_and_persist_keys()
    ...
    with st.sidebar:
        ...
        <div style="font-size:0.65rem; font-weight:600; letter-spacing:0.16em;
                    text-transform:uppercase; color:#888; margin-bottom:1rem;">API Configuration</div>
        """, unsafe_allow_html=True)

        for key, label, placeholder in [
            ("GEMINI_API_KEY",    "Gemini API Key",    "AIza..."),
            ("NOTION_TOKEN",      "Notion Token",      "secret_..."),
            ("NOTION_DATABASE_ID","Notion Database ID","xxxxxxxx-..."),
        ]:
            st.markdown(f"""
            <div style="font-size:0.7rem; font-weight:500; letter-spacing:0.04em;
                        color:#555; margin-bottom:0.2rem;">{label}</div>
            """, unsafe_allow_html=True)
            # key= binds to session state; value is pre-populated by init_session_keys()
            st.text_input(
                label,
                type="password",
        ...
        if just_saved:
            ...
            <div style="font-size:0.7rem; font-weight:600; letter-spacing:0.1em;
                        text-transform:uppercase; color:#2D6A4F; margin:0.4rem 0 0.8rem 0;">
                ✓ 저장되었습니다
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("<div style='margin-bottom:0.8rem'></div>", unsafe_allow_html=True)

        st.markdown("<hr style='margin:0.5rem 0 1rem 0; border-color:#C8C5BE;'>", unsafe_allow_html=True)
        ...
        st.markdown("<hr style='margin:1.5rem 0 1rem 0; border-color:#C8C5BE;'>", unsafe_allow_html=True)
        st.markdown(f"""
        <div style="font-size:0.62rem; color:#AAA; line-height:1.9;">
            키는 <code style="font-size:0.6rem; background:#E8E6E0; padding:0.1rem 0.3rem;">
            {SECRETS_FILE}</code>에 자동 저장됩니다.
        </div>
        """, unsafe_allow_html=True)
...
def send_to_notion(journal: dict, token: str, database_id: str):
    ...
    properties = {
        ...
        "클래스": {"rich_text": _rt(journal["class_name"])},
...
def render_upload_tab(class_id: str, class_name: str, keys: dict):
    ...
    if sc2.button(
        ...
        help="사이드바에서 Notion 키를 설정해 주세요." if not notion_ready else "",
    ):
        cdata_fresh = load_class_data(class_id)
        sent = []
        errors = []
...
def render_class_detail(class_id: str, class_name: str):
    ...
    if st.button("← Back"):
        for k in ("nav_class_id", "nav_class_name", "_class_result"):
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    init_session_keys()   # load persisted keys before anything renders
    render_sidebar()
